...
export default function MovieCard({ movie }: MovieCardProps) {
  return (
    <Link href={`/detail/${movie.movieCd}`}>
      <div className="bg-gradient-to-br from-zinc-900 to-zinc-800 p-5 sm:p-6 rounded-xl border border-white/10 hover:border-red-500/50 transition-all duration-300 hover:shadow-lg hover:shadow-red-500/20 cursor-pointer hover:scale-105">
      <h3 className="text-lg sm:text-xl font-bold text-white mb-2">{movie.movieNm}</h3>
      {movie.movieNmEn && (
        <p className="text-gray-400 text-xs sm:text-sm mb-3">{movie.movieNmEn}</p>
      )}
